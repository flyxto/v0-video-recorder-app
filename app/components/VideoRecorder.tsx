...
import { Camera, Square, AlertCircle, RefreshCw, Download } from 'lucide-react'
...
export default function VideoRecorder() {
  ...
  const drawOverlays = (ctx: CanvasRenderingContext2D) => {
    ...
    const activeOverlay = OVERLAYS.find(overlay => 
      elapsed >= overlay.startTime && elapsed < overlay.startTime + overlay.duration
    )
        ...
        <div className="text-center max-w-md">
          ...
          {process.env.NODE_ENV === 'development' && cameraReady && (
            <div className="absolute bottom-4 left-4 bg-black bg-opacity-75 text-white text-xs p-2 rounded">
              ...
              <div>Animation: {animationRef.current ? 'Running' : 'Stopped'}</div>
            </div>
          )}
        </div>
      </div>
